...
<div class="main-content">
  ...
  <form id="ausleihen" method="post" action="Logbuch-Ausleihen.jsp">
    ...
    <select class="form-control" name="code" size="1">
      ...
      <option value="${availiabelCodes.code}"
              <c:if test="${param.code eq availiabelCodes.code}">selected="selected"</c:if>>
    ...
    <select class="form-control" name="personalNBR" size="1">
      ...
      <option value="${availiabelANG.Angestelltennummer}"
              <c:if test="${param.Angestelltennummer eq availiabelANG.Angestelltennummer}">selected="selected"</c:if>>
  ...
  <c:if test="${!empty booklog_id}">
    ...
    <sql:query var="bookdBooks"
               sql="SELECT code, LOGBUCHENTLEHNER from Raumschiff Where LOGBUCHENTLEHNER is not null and LOGBUCHENTLEHNER = ? ">
      <sql:param value="${booklog_id}"/>
    </sql:query>
