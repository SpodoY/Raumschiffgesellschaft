...
<c:if test="${!empty param.menu }">
    ...
    <c:if test="${!empty param.sozialversicherungsnummer && !empty param.vorname && !empty param.nachname && !empty param.telefonnummer && !empty param.strasse && !empty param.hausnummer && !empty param.postleitzahl && !empty param.ort}">
        ...
        <c:catch var ="catchShowPersonException">
            <sql:query var="angelegtePerson" sql="SELECT Person.*, Hat_Telefonnummer.Telefonnummer FROM Person JOIN Hat_Telefonnummer ON Person.Sozialversicherungsnummer = Hat_Telefonnummer.Sozialversicherungsnummer WHERE Person.Sozialversicherungsnummer  Like ?" >
                <sql:param value="${person_id}" />
            </sql:query>
        ...
        <br>
        <br>
        <button onclick="window.location.href = 'Techniker-Anlegen2.jsp';">Continue with entering the employee data
        </button>
