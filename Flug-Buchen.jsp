            ...
            <%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
                ...
                <body>
                    <nav class="navbar navbar-inverse">
                        <div class="container-fluid">
                            <ul class="nav navbar-nav">
                                ...
                                <li class="active"><a href="Flug-Buchen.jsp">Flug Buchen</a></li>
                    ...
                    <div class="main-content">
                        ...
                        <sql:query var="tables" sql="select * from Flug">
                        </sql:query>
                        ...
                        <form id="myForm" method="post" action="Flug-Buchen.jsp">
                            ...
                            <select class="form-control" name="Person" size="1">
                                ...
                                <sql:query var="personen" sql="select Passagiernummer from Passagier">
                                </sql:query>

                                <c:forEach var="person" begin="0" items="${personen.rows}">
                                    <option value="${person.Passagiernummer}" <c:if
                                        test="${param.Passagiernummer eq person.Passagiernummer}">selected="selected"
                                        </c:if>>
                            ...
                            <select class="form-control" name="Flugnummer" size="1">
                                ...
                                <sql:query var="Flugnum" sql="select Flugnummer from Flug">
                                </sql:query>

                                <c:forEach var="Flugnum" begin="0" items="${Flugnum.rows}">
                                    <option value="${Flugnum.Flugnummer}" <c:if
                                        test="${param.Flugnummer eq Flugnum.Flugnummer}">selected="selected"</c:if>>
                                        ${Flugnum.Flugnummer}
                                    </option>
                                </c:forEach>
                            </select>
                        ...
                        <c:if test="${!empty param.menu}">
                            ...
                            <sql:query var="alreadyBooked"
                                sql="select count(*) as amount from Bucht where Flugnummer = ? and Passagiernummer = ?">
                                <sql:param value="${param.Flugnummer}" />
                                <sql:param value="${param.Person}" />
                            </sql:query>

                            <c:if
                                test="${!empty param.Person && !empty param.Flugnummer && !empty param.Klasse && alreadyBooked.rows[0].amount == 0}">
                            ...
                            <c:if test="${alreadyBooked.rows[0].amount > 0}">
                                <h2 style="color: red;"> You seem to have already booked this flight! </h2>
                            </c:if>
                        ...
                        <c:if test="${!empty buchungs_id}">
                            ...
                            <sql:query var="fluege" sql="select * from flug f where f.Flugnummer = ?">
                                        ...
                                        <td>
                                            <form method="post" action="Flug-Details.jsp">
                                                <input type="hidden" name="source"
                                                    value="${fluege.rows[0].Abflugplanet}" />
                                                ...
                                                <input type="hidden" name="depTime"
                                                    value="${fluege.rows[0].Abflugzeit}" />
                                                <input type="hidden" name="arrTime"
                                                    value="${fluege.rows[0].Ankunftszeit}" />
