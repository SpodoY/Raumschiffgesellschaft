...
<form id="myForm" method="post" action="Flug-Buchen.jsp" >
    ...
    <select class="form-control" name="Person" size="1">
		...
		<c:forEach var="person" begin="0" items="${personen.rows}">
			<option value="${person.Passagiernummer}" <c:if test="${param.Passagiernummer eq person.Passagiernummer}">selected="selected"</c:if>>
	...
	<select class="form-control" name="Flugnummer" size="1">
		...
		<c:forEach var="Flugnum" begin="0" items="${Flugnum.rows}">
			<option value="${Flugnum.Flugnummer}" <c:if test="${param.Flugnummer eq Flugnum.Flugnummer}">selected="selected"</c:if>>
            	${Flugnum.Flugnummer}
            </option>
		</c:forEach>
	</select>
...
<c:if test="${!empty param.menu }">
    ...
    <c:if test="${!empty param.Person && !empty param.Flugnummer && !empty param.Klasse  }">
        ...
        <c:set var = "buchungs_id" scope = "session" value = "<%= java.util.UUID.randomUUID().toString().replaceAll(\"-\", \"\").substring(0, 30) %>"/>

        <sql:update  var="buchen" sql="INSERT INTO Bucht (Buchungsnummer, Klasse, Buchungsdatum, Flugnummer, Passagiernummer) VALUES (?,?,SYSDATE,?,?)">
        ...
        <p> Erfolgreich gebucht mit der Buchnungsnummer : "${buchungs_id}"" </p>
